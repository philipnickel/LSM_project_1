...
if rank == 0:
    ...
    for source_rank in range(1, size):
        ...
        for _ in range(num_chunks):
            ...
            all_metadata.append((source_rank, start_row, end_row))

    # Phase 2: Post ALL non-blocking receives at once
    pending_receives = []  # Store (request, start_row, end_row, buffer)
    
    for source_rank, start_row, end_row in all_metadata:
        # Allocate buffer for this chunk
        chunk_rows = end_row - start_row
        chunk_data = np.zeros((chunk_rows, image_size[1]))
        
        # Post non-blocking receive
        req = comm.Irecv(chunk_data, source=source_rank, tag=2)
        
        # Store request with metadata
        pending_receives.append((req, start_row, end_row, chunk_data))
    
    print(f"Rank 0: Posted {len(pending_receives)} Irecv operations")
    
    # Phase 3: Wait for all receives and assemble image
    for req, start_row, end_row, chunk_data in pending_receives:
        req.Wait()
        full_image[start_row:end_row, :] = chunk_data
    
    print("Rank 0: All data received")
    
else:
    # Phase 1: Send all metadata 
    ...
    requests = []
    for start_row, end_row, chunk_data in results:
        req = comm.Isend(chunk_data, dest=0, tag=2)
        requests.append(req)
...
if rank == 0:
    ...
    plt.title(f"Mandelbrot Set - MPI Non-blocking Static Implementation ({size} processes)")

    plt.savefig("Plots/non_blocking_static_mandelbrot.pdf", bbox_inches="tight", pad_inches=0)
